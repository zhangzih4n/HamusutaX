...
class EbmlDecoder(private val source: Source) {
    fun readPair() =
    ...
    fun readId() =
        readInteger(IntegerType.ID)

    /**
     * @param type 整数数据类型。作为字节序列长度时会移除最高位的 1。作为 ID 时无需移除
     */
    fun readInteger(type: IntegerType = IntegerType.BYTES_LENGTH): Long {
        ...
        while (remainBytes > 0) {
            ...
            if (isReadingLength) {
                if (byte == 0.toByte()) {
                    ...
                    if (type == IntegerType.BYTES_LENGTH) {
                        buffer.writeByte(byte.clearHighestOneBit())
                        continue
                    }
                }
            }
